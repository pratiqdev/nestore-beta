...
const splitPath = (path) => path.split(/\[|\]\[|\]\.|\.|\]|\//g);
//                                 path.split(/\[|\]\[|\]\.|\.|\]|\//g)
const getLastKeyFromPathString = (path) => {
...
class Nestore extends EE2 {
    ...
    constructor(store = {}, options = {}) {
        ...
        //   }
        // })
        const storeOmitted = Object.fromEntries(Object.entries(store)
            .filter(([KEY]) => !this.#CUSTOM_MUTATORS
            .includes(KEY)));
            ...
            if (typeof val === 'function') {
                if (key.startsWith('$')) {
                    ...
                    try {
                        ...
                        if (RESERVED_KEYS.some((reserved) => key === reserved)) {
                            log(`The key "${key}" is reserved for nestore. Please use a different name.`);
                        }
                        // let newStruct = {
                        //     loading: this.#adapter_loadingData,
    ...
    #convertStringOrArrayToNormalizedPathString(path) {
        ...
        const pathResult = split.join(this.#DELIMITER_CHAR);
        // log.norm(`Normalized path: ${path} => ${_path}`)
        return pathResult;
    }
    #updateDevtoolFromStore = (path, originalValue, value, data) => {
        let safeOriginalValue, safeValue, safePath;
        try {
